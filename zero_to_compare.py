...
from layer_utils import getResultMatrix,getResultMatrixWithCa, matrixForLayer, matrixForPeriod
...
def getListOfDataForAngle(alfa):
  ...
  for (lamda, n1, n2, ns) in zip(lamdaList, PVKDataList, CADataList, glassDataList):
    ...
    [h1, h2, hForLastLayer, d] = [50, 150, 175, 10]
    """первый , второй слои и слой стекла """
    [layerFirst, layerSecond, glassLayer, CALastLayer] = [Layer(lamda, n1, h1,alfa), Layer(lamda, n2, h2,alfa),Layer(lamda, ns, 1000000,alfa),Layer(lamda, n2,hForLastLayer,alfa)]
   
    """allLayersMatrix - матрица всех периодов PSMatrix - последний слой стекло  """ 
     # allMatrixWithLastLayer =np.dot(allLayersMatrix,lastLayerMatrix)
    """ getResultMatrixWithCa вместо getResultMatrix, поскольку добавлен слой CA между воздухом и другим: """
    allLayersMatrix, PSMatrix = (getResultMatrixWithCa([layerFirst, layerSecond], CALastLayer, d), getPMatrix(layerFirst.alfa0, ns))
    ...
    plotListForAngle.append(T.real)
...
zeroTDataTheor = getListOfDataForAngle(0)

normilizeZeroTData = zeroTData
zeroTDataTheor = pyd.chaining.chain(zeroTDataTheor).map(lambda x:x*100).value()
plt.plot(lamdaList, zeroTDataTheor, linestyle='--')
plt.plot(lamdaList, normilizeZeroTData, linestyle='solid')
plt.show()

# plt.plot(lamdaList, zeroTData, 'green')
